...
SCHEMA="archie-voxel-heldout/v3"
...
def briefs(count=3,base_seed=731003):
    ...
    for i in range(max(1,int(count))):
        seed=int(base_seed)+i*7919;name=f"Heldout-{hashlib.sha256(str(seed).encode()).hexdigest()[:6]}";text=BRIEF_FAMILIES[i%len(BRIEF_FAMILIES)].format(name=name,seed=seed);required=("offline","world","movement","mouse_look","jump_gravity","collision","remove_place","materials","persistence","render_loop");bid=hashlib.sha256((str(seed)+"\0"+text).encode()).hexdigest()[:16];out.append(Brief(bid,seed,text,required))
    return out
def _all_text(root):
    chunks=[];files=[]
...
def evaluate(config,root,brief):
    run=run_maker(config,brief.text,root);static=grade(root);runtime=voxel_runtime.execute(root);passes=run["payload"]["status"]=="FINISHED" and bool(static["payload"]["passes"]) and runtime["payload"].get("status")=="PASS";return receipt("voxel.heldout_case",{"schema":SCHEMA,"brief":asdict(brief),"maker":run["payload"],"grade":static["payload"],"runtime":runtime["payload"],"passes":passes,"claimable":passes})
def evaluate_suite(config,out_root,count=3,base_seed=731003):
    ready=probe(config)
    if ready["payload"]["status"]!="READY":return receipt("voxel.heldout_suite",{"status":"BLOCKED","reason":ready["payload"],"passes":False,"claimable":False,"base_seed":base_seed})
    selected=briefs(count=count,base_seed=base_seed);cases=[evaluate(config,out_root/b.id,b) for b in selected];passed=sum(bool(c["payload"]["passes"]) for c in cases);payloads=[c["payload"] for c in cases]
    return receipt("voxel.heldout_suite",{"status":"COMPLETE","base_seed":int(base_seed),"brief_ids":[b.id for b in selected],"cases":payloads,"passed":passed,"total":len(cases),"success_rate":passed/max(1,len(cases)),"passes":passed==len(cases),"claimable":passed==len(cases),"executed_runtime_required":True,"suite_sha256":hashlib.sha256(canonical(payloads)).hexdigest()})
def court():
    with tempfile.TemporaryDirectory(prefix="archie-voxel-grade-") as td:
        root=Path(td);root.joinpath("index.html").write_text("""<canvas></canvas><script>let world=new Uint8Array(99),player={position:0},yaw=0,pitch=0,gravity=9.8,material=1;function solid(){} function collision(){} function ray(){} addEventListener('keydown',e=>{if(e.code==='KeyW'||e.code==='Space')player.position++});addEventListener('mousemove',e=>{yaw+=e.movementX;pitch+=e.movementY});document.body.requestPointerLock;addEventListener('mousedown',()=>{ray();material=2});function save(){localStorage.setItem('w','x')}function load(){localStorage.getItem('w')}function loop(){requestAnimationFrame(loop)}loop();</script>""",encoding="utf-8");g1=grade(root);g2=grade(root);a=briefs(4,111);b=briefs(4,222);unique=len({x.id for x in a})==4;fresh={x.id for x in a}.isdisjoint({x.id for x in b});no_ref=all("ARCHIE VOXEL" not in x.text for x in a+b);passes=verify_receipt(g1) and g1==g2 and bool(g1["payload"]["passes"]) and unique and fresh and no_ref;return receipt("voxel_heldout.court",{"passes":passes,"grader_deterministic":g1==g2,"brief_ids_unique":unique,"fresh_seed_sets_disjoint":fresh,"reference_name_absent":no_ref,"success_contract":"static grade + executed browser runtime PASS"})
