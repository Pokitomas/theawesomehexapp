...
def wrap(kind: str, module, key: str) -> dict:
    r = module.court()
    return receipt(kind, {key: r["payload"], "passes": bool(r["payload"].get("passes"))})


def cold_start_court() -> dict:
    modules = "core,maker_fixture,video_editor_v2,render_ffmpeg,render_integration,synthetic_pref,preference_train,distill,voxel_game,corpus_foundry,curriculum,study_index,model_sourcing,model_tournament,local_model_maker,voxel_heldout,trajectory_dataset,train_sft"
...
def run()->dict:
    courts={
        ...
        "corpus_foundry":wrap("court.corpus",corpus_foundry,"corpus"),"curriculum":wrap("court.curriculum",curriculum,"curriculum"),"study_index":wrap("court.study_index",study_index,"study_index"),"trajectory_dataset":wrap("court.trajectory_dataset",trajectory_dataset,"trajectory_dataset"),
    ...
    return receipt("kernel-maker.promotion-court",{"courts":courts,"passes":passes,"all_required_pass":all(passes.values()),"promotion":"ADMIT" if all(passes.values()) else "REFUSE","local_model_voxel_claim":"NOT_EVALUATED_BY_HOSTED_CI; only voxel_heldout.evaluate_suite on a real local endpoint may set this claim"})
