...
def cold_start_court() -> dict:
    code = (
        ...
        "import core,maker_fixture,video_editor_v2,render_ffmpeg,render_integration,synthetic_pref,preference_train,distill;"
        "print(core.SCHEMA)"
    )
...
def receipt_court() -> dict:
    ...
    corrupt = json.loads(json.dumps(value)); corrupt["payload"]["z"] = 2
...
def maker_court() -> dict:
    r = maker_fixture.court(); p = r["payload"]
    passes = all(bool(p.get(k)) for k in ("inspect_valid", "pre_repair_failed", "repair_receipt", "cache_invalidated", "post_repair_passed", "run_passed"))
    return receipt("court.maker", {"maker": p, "passes": passes})


def editor_court() -> dict:
    r = video_editor_v2.court(); p = r["payload"]; b = p["benchmark"]
    passes = all(bool(p.get(k)) for k in ("add_receipt", "split_receipt", "undo_receipt", "redo_receipt", "redo_exact", "portable_roundtrip", "save_receipt")) and bool(b["deterministic_replay"]) and bool(b["exact_frame_roundtrip"]) and bool(b["mutation_receipts_valid"])
...
def distill_court() -> dict:
    r = distill.court(); p = r["payload"]
    passes = bool(p["kl_oracle"]["passes"]) and p["import_attempt"]["status"] in {"BLOCKED", "READY_TO_MATERIALIZE"} and p["triton"]["status"] in {"SKIP", "PASS"}
...
def run() -> dict:
    courts = {
        ...
        "video_editor": editor_court(), "video_render": render_court(), "video_render_integration": render_integration_court(), "distill": distill_court(),
        "synthetic_preference": synthetic_court(), "preference_training": preference_training_court(),
    }
