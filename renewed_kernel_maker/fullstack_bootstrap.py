...
def ensure_llama_server(cache,allow_network):
    ...
    hits=list(build.rglob('llama-server'))+list(build.rglob('llama-server.exe'));return {'status':'READY','path':str(hits[0]),'built':True} if hits else {'status':'BLOCKED','reason':'built-but-server-not-found'}
def ensure_model(cache,allow_network,repo=DEFAULT_HF_REPO,filename=DEFAULT_HF_FILE):
    found=local_runtime.discover([cache/'models'])['payload']['ggufs'];choice=local_runtime.choose_gguf(found,max_gib=5.6)
    ...
    try:
        ...
        p=hf_hub_download(repo_id=repo,filename=filename,local_dir=str(dst.parent));return {'status':'READY','path':str(Path(p)),'source':repo,'gib':Path(p).stat().st_size/(1024**3)}
        ...
        except Exception as second:tmp.unlink(missing_ok=True);return {'status':'BLOCKED','reason':f'model-download-failed:{type(first).__name__}/{type(second).__name__}','detail':str(second)}
def ensure_browser(cache,allow_network):
    try:import playwright.sync_api;return {'status':'READY','source':'existing'}
    except Exception:pass
    if not allow_network:return {'status':'BLOCKED','reason':'playwright-missing'}
    deps=cache/'pydeps';browsers=cache/'playwright-browsers';deps.mkdir(parents=True,exist_ok=True);env={**os.environ,'PYTHONPATH':str(deps)+os.pathsep+os.environ.get('PYTHONPATH',''),'PLAYWRIGHT_BROWSERS_PATH':str(browsers)}
