...
def symmetric_wrong(value: torch.Tensor | None) -> torch.Tensor | None:
    if value is None:
        return None
    if value.ndim >= 2 and value.size(1) > 1:
        return torch.roll(value.detach(), shifts=1, dims=1)
    return -value.detach()
...
def pursuit_contract(
    ...
    return {
        "schema": CONTRACT_SCHEMA, "method": METHOD, "model": asdict(cfg),
        "source": {
            "language_shell_sha256": source_sha, "plan_sha256": plan_receipt["plan_sha256"],
        ...
        "streaming": {
            ...
            "cache_bytes": args.cache_bytes, "lookahead": args.pursuit_lookahead,
            "prefetch_workers": args.prefetch_workers,
        },
        "pursuit": {
            "counterfactual_every": args.counterfactual_every, "state_margin": args.state_margin,
        ...
        "optimization": {
            ...
            "language_lr_scale": args.language_lr_scale, "maximum_steps": args.max_steps,
            "warmup_steps": args.warmup_steps, "gradient_clip": args.grad_clip,
        },
        "execution": {
