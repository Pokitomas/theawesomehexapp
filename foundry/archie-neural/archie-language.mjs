...
export function validateArchieProgram(program) {
  ...
  for (const instruction of instructions) {
    ...
    graph.set(instruction.id, instruction.after || []);
...
export function compileArchieProgram(program) {
  ...
  const executableKinds = new Set(['step', 'verify', 'learn', 'halt']);
  const executable = instructions.filter(item => executableKinds.has(item.kind));
  const pending = new Map(executable.map(item => [item.id, item]));
  const emitted = new Set();
  const schedule = [];
  while (pending.size) {
    const ready = [...pending.values()]
      .filter(item => (item.after || []).every(id => !executableKinds.has(byId.get(id)?.kind) || emitted.has(id)))
  ...
  return Object.freeze({
    ...
    world: instructions.filter(item => !executableKinds.has(item.kind) && item.kind !== 'presentation'),
    schedule,
