...
def effect_signature(delta: dict[str, Any]) -> dict[str, Any]:
    """Path-invariant quotient of an observed transition.

    This is deliberately primitive.  It groups transformations by structural
    effect rather than by command spelling, giving a baseline latent action
    alphabet that can later be replaced by a learned codebook.
    """
...
class Action:
    ...
    def public(self) -> dict[str, Any]:
        # Do not expose restoration bytes as the motor primitive.
        safe = {k: v for k, v in self.args.items() if not k.startswith("_")}
        return {"kind": self.kind, "args": safe}
...
def apply_action(root: Path, action: Action) -> Action:
    ...
    if kind == "write":
        ...
        return Action("unlink", {"path": args["path"], "_restore": args["text"]})
...
def choose_action(root: Path, rng: random.Random, nonce: int) -> Action:
    ...
    empty_dirs = [p for p in dirs if not any(q != p and q.startswith(p + "/") for q in state)]
    ...
    if kind == "rename":
        ...
        return Action("rename", {"src": src, "dst": f"moved/m{nonce:06d}.dat"})
    if kind == "rmdir":
        ...
        for i in range(steps):
            ...
            record = {
                ...
                "courts": {
                    ...
                    "replay_exact": replayed == after,
                    "inverse_motor": re_inverse.public(),
                },
            }
...
def main() -> None:
    ...
    try:
        summary = run_court(root, ledger, args.steps, args.seed)
