...
export function resolveHostedConfig({
  ...
  const selectedPublicUrl = absoluteHttpUrl(publicUrl || env.ARCHIED_PUBLIC_URL, 'ARCHIED_PUBLIC_URL');
  const insecureAllowed = allowInsecure === true || env.ARCHIED_ALLOW_INSECURE_HOSTED === '1';
  ...
  return Object.freeze({
    ...
    founder_token_sha256: founderTokenSha256 || env.ARCHIED_FOUNDER_TOKEN_SHA256,
    developer_token_sha256: developerTokenSha256 || env.ARCHIED_DEVELOPER_TOKEN_SHA256,
    session_key: sessionKey || env.ARCHIED_SESSION_KEY,
    secret_key: secretKey || env.ARCHIED_SECRET_KEY,
    secure_cookies: new URL(selectedPublicUrl).protocol === 'https:'
  });
}
...
export async function startHostedArchied(options = {}) {
  ...
  const descriptor = Object.freeze({
    ...
    authentication: security.descriptor,
    storage: {
  ...
  function requireIdentity(request, roles = ['founder', 'developer']) {
    return requireRole(security.authenticate(request), roles);
  }
  ...
  server.on('request', async (request, response) => {
    try {
      ...
      if (method === 'POST' && pathname === '/login') {
        ...
        const identity = security.identityForToken(token);
        if (!identity) {
          failedLogin(ip);
      ...
      if (method === 'GET' && parts[0] === 'w' && parts[1] && parts.length === 2) {
        jsonResponse(response, 200, await internal.engine.inspect(parts[1], { principalId: identity.principal_id }));
        return;
      }
      ...
      if (method === 'POST' && pathname === '/v1/hosted/shares') {
        ...
        await internal.engine.execute(body.workspace_id, identity.principal_id, 'grant.issue', {
          ...
          capabilities: ['read'],
          expires_at: body.expires_in_ms
            ? new Date(Date.now() + Number(body.expires_in_ms)).toISOString()
      ...
      if (
        ...
        jsonResponse(response, 200, {
          schema: 'archie-hosted-share-list/v1',
          workspace_id: parts[3],
          shares: await shareRegistry.list(parts[3])
        });
        return;
      }
      ...
      if (method === 'PUT' && parts[0] === 'v1' && parts[1] === 'hosted' && parts[2] === 'secrets' && parts[3] && parts.length === 4) {
        ...
        const body = await readJsonBody(request);
        jsonResponse(response, 200, {
          schema: 'archie-hosted-secret-write/v1',
          secret: await secretStore.set(parts[3], body.value)
        });
        return;
      }
