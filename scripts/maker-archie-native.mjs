...
export function resolveNativeArchiePaths(repoRoot, { env = process.env, home = os.homedir() } = {}) {
  ...
  return Object.freeze({
    root,
    model_path: path.join(root, 'models', 'native-maker-plans.json')
  });
}
...
export function normalizeReusableMakerPlan(value) {
  ...
  const ownedPaths = [...new Set((Array.isArray(value.owned_paths) ? value.owned_paths : [])
...
export async function recallNativeMakerPlan({
  repoRoot,
  request,
  baseBranch = 'main',
  env = process.env,
  ...
  try {
    const result = await brain.plan({
      ...
      instruction,
      context: { repository: path.basename(path.resolve(repoRoot)), base_branch: clean(baseBranch, 200) }
    }, { allow_teacher: false });
    ...
    return Object.freeze({
      ...
      reason: plan ? null : `archie state ${result.state}`
    });
  } catch (error) {
    const message = clean(error?.message || error, 2000);
    if (/at least one completed archie distillation example is required/i.test(message)) {
      return Object.freeze({ status: 'miss', plan: null, root: paths.root, reason: 'empty corpus' });
    }
    return Object.freeze({ status: 'failed', plan: null, root: paths.root, reason: message });
  }
}

export async function rememberNativeMakerRun({
  repoRoot,
  receipt,
  env = process.env,
  home = os.homedir(),
