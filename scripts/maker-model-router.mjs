...
function evidenceValue(input, normalize, fallbackEvidence = 'unknown') {
  if (isObject(input) && ('value' in input || 'evidence' in input)) {
    const evidence = EVIDENCE.has(clean(input.evidence, 30).toLowerCase()) ? clean(input.evidence, 30).toLowerCase() : fallbackEvidence;
...
export function normalizeProviderDescriptor(input = {}) {
  ...
  const locality = evidenceValue(input.locality, value => enumValue(value, LOCALITIES));
  const privacy = evidenceValue(input.privacy, value => clean(value || 'unknown', 100).toLowerCase());
  const latency = evidenceValue(input.latency_class, value => LATENCY_RANK[clean(value, 50).toLowerCase()] === undefined ? 'unknown' : clean(value, 50).toLowerCase());
  ...
  const descriptor = {
    ...
    kind,
    availability: enumValue(input.availability, AVAILABILITY),
    availability_evidence: EVIDENCE.has(clean(input.availability_evidence, 30).toLowerCase()) ? clean(input.availability_evidence, 30).toLowerCase() : (input.availability === undefined ? 'unknown' : 'configured'),
    capabilities: {
    ...
    limits: {
      context_tokens: evidenceValue(limits.context_tokens, integerValue),
      output_tokens: evidenceValue(limits.output_tokens, integerValue)
    },
    latency_class: latency,
    privacy,
    locality,
    region: evidenceValue(input.region, value => clean(value || 'unknown', 100).toLowerCase()),
    cost: {
      input_per_million: evidenceValue(cost.input_per_million, moneyValue),
      output_per_million: evidenceValue(cost.output_per_million, moneyValue)
    },
    admission: {
    ...
    public: {
      engine_label: normalizePublicLabel(input.public?.engine_label, kind === 'native_checkpoint' ? 'Local Maker intelligence' : 'Adaptive Maker intelligence'),
...
export function createModelRouter({
  ...
  return Object.freeze({ route, grade, probe, probeAll, health, budget: () => ledger.snapshot() });
}
