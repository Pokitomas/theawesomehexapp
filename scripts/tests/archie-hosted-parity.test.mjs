...
test('hosted Archie protects one product and preserves exact state across restart', async t => {
  ...
  assert.equal(restartedStatus.workspaces.items[0].head_digest, journey.head_digest);
  ...
  assert.equal((await persistentShare.json()).workspace.head_digest, journey.head_digest);
});
