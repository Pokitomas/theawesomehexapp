...
import { runHybridRunnerOnce } from '../archie-hybrid-runner.mjs';
...
async function login(runtime) {
  const response = await fetch(new URL('/auth/login', runtime.url), {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ token: founderToken })
  });
  assert.equal(response.status, 200);
  const value = await response.json();
  const cookie = response.headers.get('set-cookie')?.split(';')[0] || '';
  assert.match(cookie, /^archie_founder_session=/);
  return { cookie, csrf: value.csrf_token };
}

function founderHeaders(auth, extra = {}) {
  return { cookie: auth.cookie, 'x-archie-csrf': auth.csrf, ...extra };
}

async function founderJson(runtime, auth, pathname, { method = 'GET', body = null } = {}) {
  const response = await fetch(new URL(pathname, runtime.url), {
    method,
    headers: founderHeaders(auth, body === null ? {} : { 'content-type': 'application/json' }),
...
test('hosted Archie controls one outbound-only local runner through exact fenced receipts', async t => {
  const home = await temporary(t, 'archie-hybrid-hosted-');
  const runnerRoot = await temporary(t, 'archie-hybrid-local-');
  const runtime = await startHostedArchied({
    config: {
      home,
      host: '127.0.0.1',
      port: 0,
  ...
  const forbidden = await founderJson(runtime, auth, '/v1/hybrid/founder/offers', {
    method: 'POST',
    body: {
  ...
  const enrollmentResult = await founderJson(runtime, auth, '/v1/hybrid/founder/enrollments', {
    method: 'POST',
  ...
  const offered = await founderJson(runtime, auth, '/v1/hybrid/founder/offers', {
    method: 'POST',
    body: offer({
  ...
  assert.equal(await fs.readFile(path.join(runnerRoot, 'workspace', 'output', 'result.json'), 'utf8'), completeContent);
  ...
  const registryText = await fs.readFile(path.join(home, 'standalone', 'hybrid', 'registry.json'), 'utf8');
  ...
  const usedEnrollment = await fetch(new URL('/v1/hybrid/runner/enroll', runtime.url), {
    ...
    body: JSON.stringify({
      ...
      advertisement: JSON.parse((await fs.readFile(stateFile, 'utf8'))).advertisement
    })
  });
  ...
  const failureOffer = await founderJson(runtime, auth, '/v1/hybrid/founder/offers', {
    method: 'POST',
    body: offer({
      workspaceId: 'workspace_hybrid',
      taskId: 'task_failure',
  ...
  assert.equal(await fs.stat(path.join(runnerRoot, 'workspace', 'output', 'failure.json')).then(() => true, () => false), false);
  ...
  const status = await founderJson(runtime, auth, '/v1/hybrid/founder/status');
...
test('the runner implementation opens no inbound listener and exposes one obvious CLI', async () => {
  ...
  assert.equal(packageJson.bin['archie-hybrid-runner'], 'scripts/archie-hybrid-runner.mjs');
  assert.equal(packageJson.scripts['archie:runner'], 'node scripts/archie-hybrid-runner.mjs');
});
