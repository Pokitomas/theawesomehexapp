...
_META_INITIATIVE = re.compile(r"\b(?:i(?:'ll| will)\s+(?:choose|decide)|let\s+me\s+(?:choose|decide)|okay|sure)\b", re.I)
_DELEGATION_PATTERNS = tuple(re.compile(pattern, re.I) for pattern in (
    r"\byou\s+choose\b", r"\byou\s+decide\b", r"\bup\s+to\s+you\b",
    r"\bcan\s+you\s+(?:choose|decide)\b", r"\bcould\s+you\s+(?:choose|decide)\b",
...
_PREFERENCE_QUESTION_PATTERNS = tuple(re.compile(pattern, re.I) for pattern in (
...
def asks_user_to_choose(text: str) -> bool:
    value = normalize(text)
    return bool(value and any(pattern.search(value) for pattern in _PREFERENCE_QUESTION_PATTERNS))
...
def has_substantive_initiative(text: str) -> bool:
    ...
    if not value or asks_user_to_choose(value):
        return False
    return len(_WORD.findall(_META_INITIATIVE.sub(" ", value))) >= 3
...
def repair_delegated_candidate(candidate_reply: str) -> str:
    ...
    repaired = normalize(" ".join(part for part in parts if not asks_user_to_choose(part)))
...
def initiative_directive(current_user: str, history: Sequence[dict[str, str]] = ()) -> str:
    ...
    return (
        ...
        "Do not ask the user what they want, prefer, or find interesting; that would return the delegated decision and make zero objective progress. "
        "A question is allowed only if a missing fact is genuinely required for safety or an irreversible external action. "
