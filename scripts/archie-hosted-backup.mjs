...
export function verifyHostedBackup(value) {
  ...
  if (value.workspace_count !== value.workspaces.length) throw new WorkspaceError('Hosted backup workspace count mismatch.');
  let artifactCount = 0;
...
export async function createHostedBackup({
  ...
  const body = {
    ...
    claim_boundary: 'This backup contains exact digest-chained workspace events, admitted artifact bytes, and only encrypted secret envelopes. The external encryption and session keys are intentionally absent.'
  };
...
export async function hostedBackupStatus(dataRoot) {
  ...
  let names;
  try {
    names = (await fs.readdir(backupRoot)).filter(name => name.endsWith('.archie-backup.json')).sort().reverse();
  } catch (error) {
