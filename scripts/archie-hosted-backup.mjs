...
export function verifyHostedBackup(value) {
  ...
  if (value.backup_digest !== backupDigest(value)) throw new WorkspaceError('Hosted backup digest mismatch.');
  if (!Array.isArray(value.workspaces)) throw new WorkspaceError('Hosted backup requires a workspace array.');
...
export async function createHostedBackup({
  ...
  const body = {
    ...
    claim_boundary: 'This backup contains exact digest-chained workspace events, admitted artifact bytes, and only encrypted secret envelopes. The external encryption key is intentionally absent.'
  };
  const backup = Object.freeze({ ...body, backup_digest: backupDigest(body) });
...
export async function hostedBackupStatus(dataRoot) {
  ...
  const latestPath = path.join(backupRoot, names[0]);
  const latest = verifyHostedBackup(JSON.parse(await fs.readFile(latestPath, 'utf8')));
