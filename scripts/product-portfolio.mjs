...
function assertBand(errors, label, band, { zeroAllowed = true } = {}) {
  if (!Array.isArray(band) || band.length !== 2 || !band.every(Number.isFinite)) {
  ...
  if (minimum < 0 || (!zeroAllowed && minimum === 0)) errors.push(`${label} contains an invalid negative or zero value.`);
}
...
export function validatePortfolio(portfolio) {
  ...
  for (const [id, surface] of Object.entries(surfaces || {})) {
    ...
    if (!Array.isArray(surface?.promotion_gates) || !surface.promotion_gates.length) errors.push(`surfaces.${id}.promotion_gates must be non-empty.`);
    ...
    const hasRoute = routes.some(route => route.surface === id);
    if (!hasRoute && surface.visibility !== 'withheld-until-evidence') {
      errors.push(`surfaces.${id} has no route and is not explicitly withheld.`);
    }
    if (surface.visibility === 'withheld-until-evidence' && hasRoute) {
      errors.push(`surfaces.${id} is withheld but still claims a public route.`);
    }
  }
...
export function resolveRoute(portfolio, routePath) {
  ...
  return portfolio.routes.find(route => route.path === normalized) || null;
}

export function buildAdmissionReceipt(portfolio) {
  validatePortfolio(portfolio);
  const visibleSurfaceIds = [...new Set(portfolio.routes.map(route => route.surface))].sort();
  const withheldSurfaceIds = Object.entries(portfolio.surfaces)
    .filter(([, surface]) => surface.visibility === 'withheld-until-evidence')
    .map(([id]) => id)
    .sort();
  const body = {
    ...
    visible_surfaces: visibleSurfaceIds,
    withheld_surfaces: withheldSurfaceIds,
...
export function runCli(argv = process.argv.slice(2), output = process.stdout, errorOutput = process.stderr) {
  ...
  if (command === 'inspect') {
    output.write(`${JSON.stringify({
      decision: portfolio.portfolio_decision,
      routes: portfolio.routes,
      retirements: portfolio.retirements,
      admission: buildAdmissionReceipt(portfolio)
    }, null, 2)}\n`);
    return 0;
  }
