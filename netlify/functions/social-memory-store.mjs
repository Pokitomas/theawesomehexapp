...
export function createMemorySocialStore() {
  ...
  const safeEventUser = bundle => bundle ? { id: bundle.id, status: bundle.status, createdAt: bundle.createdAt, profile: clone(bundle.profile) } : null;
  ...
  const postView = (post, viewerId = '') => {
    ...
    return {
      ...
      engagement: {
        likes: likeCount,
        replies: replyCount,
        viewerLiked: Boolean(viewerId && state.reactions.has(reactionKey(viewerId, post.id)))
      }
    };
  };
  ...
  return Object.freeze({
    ...
    async createUser({ email, passwordHash, profile, idempotencyKey = '' }) {
      ...
      state.emails.set(normalizedEmail, id);
      state.handles.set(profile.handle, id);
      state.profiles.set(id, publicProfile);
      const result = userBundle(id);
      appendEvent({ actorId: id, type: 'account.created', objectType: 'user', objectId: id, payload: { handle: profile.handle }, idempotencyKey, response: safeEventUser(result) });
      return clone(result);
    },
    async findUserByEmail(email) {
      const id = state.emails.get(String(email).toLowerCase());
      return id ? clone(state.users.get(id)) : null;
    },
    async getUserById(id) { return clone(userBundle(id)); },
    async getUserByHandle(handle) {
      const id = state.handles.get(String(handle).toLowerCase());
      return id ? clone(userBundle(id)) : null;
    },
    async updateProfile(userId, patch, idempotencyKey = '') {
      const replay = eventResponse(userId, idempotencyKey);
      ...
      if (patch.handle && patch.handle !== current.handle) {
        ...
        state.handles.delete(current.handle);
        state.handles.set(patch.handle, userId);
      }
      const next = { ...current, ...clone(patch), updatedAt: nowISO() };
      state.profiles.set(userId, next);
      const result = userBundle(userId);
      appendEvent({ actorId: userId, type: 'profile.updated', objectType: 'profile', objectId: userId, payload: patch, idempotencyKey, response: safeEventUser(result) });
      return clone(result);
    },
    async createSession({ userId, accessHash, refreshHash, accessExpiresAt, refreshExpiresAt }) {
      ...
      state.access.set(accessHash, id);
      state.refresh.set(refreshHash, id);
      return clone(session);
    },
    async sessionByAccessHash(hash) {
      const id = state.access.get(hash);
      return id ? clone(state.sessions.get(id)) : null;
    },
    async sessionByRefreshHash(hash) {
      const id = state.refresh.get(hash);
      return id ? clone(state.sessions.get(id)) : null;
    },
    async rotateSession(sessionId, { accessHash, accessExpiresAt, refreshHash, refreshExpiresAt }) {
      const session = state.sessions.get(sessionId);
      if (!session) return null;
      state.access.delete(session.accessHash);
      state.refresh.delete(session.refreshHash);
      Object.assign(session, { accessHash, accessExpiresAt, refreshHash, refreshExpiresAt });
      state.access.set(accessHash, sessionId);
      state.refresh.set(refreshHash, sessionId);
      return clone(session);
    },
    ...
    async createPost(actorId, input, idempotencyKey = '') {
      const replay = eventResponse(actorId, idempotencyKey);
      if (replay) return replay;
    ...
    async setFollow(actorId, followedId, active, idempotencyKey = '') {
      ...
      if (active) state.follows.set(key, { followerId: actorId, followedId, createdAt: nowISO() });
    ...
    async setReaction(actorId, postId, active, kind = 'like', idempotencyKey = '') {
      ...
      if (active) state.reactions.set(key, { actorId, postId, kind, createdAt: nowISO() });
    ...
    async followingFeed(viewerId, { cursor = null, limit = 30 } = {}) {
      const followed = new Set([viewerId]);
      for (const item of state.follows.values()) if (item.followerId === viewerId) followed.add(item.followedId);
      const rows = [...state.posts.values()]
        .filter(item => followed.has(item.authorId) && item.visibility === 'public' && !item.deletedAt)
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt) || b.id.localeCompare(a.id))
        .map(item => postView(item, viewerId));
      return page(rows, cursor, limit);
    },
    async userPosts(userId, viewerId, { cursor = null, limit = 30 } = {}) {
      const rows = [...state.posts.values()]
        .filter(item => item.authorId === userId && item.visibility === 'public' && !item.deletedAt)
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt) || b.id.localeCompare(a.id))
        .map(item => postView(item, viewerId));
      return page(rows, cursor, limit);
    },
    async thread(postId, viewerId, { cursor = null, limit = 30 } = {}) {
      const root = state.posts.get(postId);
      if (!root) return null;
      const ancestors = [];
      let current = root;
      const seen = new Set();
      while (current?.replyToId && !seen.has(current.replyToId)) {
        seen.add(current.replyToId);
        current = state.posts.get(current.replyToId);
        if (current) ancestors.unshift(postView(current, viewerId));
      }
      const replies = [...state.posts.values()]
        .filter(item => item.replyToId === postId && !item.deletedAt)
