...
## Archie portable derivational runtime

The Maker app is published at `https://pokitomas.github.io/theawesomehexapp/maker/`. Its local Archie layer now treats intelligence as reusable connections between typed state transformations rather than whole-plan phrase matching. Successful trajectories compile into an integrity-bound sparse graph, quantized relation matrix, domain adapters, preconditions, effects, invariants, and proof-carrying plans. Lexical matching remains a candidate signal, not the sole admission rule.

Archie can compose previously unseen combinations of admitted skills and can ask a teacher for one minimal missing domain edge rather than relearning an entire workflow. The production UI exposes derivation mode, graph size, proof digest, and unresolved missing edges without claiming that the browser executed or trained work unless an authenticated runtime returns a receipt.

Run the complete local proof surface with:

```bash
npm run verify:archie-intelligence
```

See [`ARCHIE_DERIVATION.md`](./ARCHIE_DERIVATION.md) for the representation, learning loop, portability boundary, and benchmark semantics.
