...
The unauthenticated `/health` endpoint exposes only liveness, hosted mode, service version, and migration level. The product surface, full descriptor, stable workspace URLs, workspace APIs, operational status, exports, secrets metadata, and backups require authentication. Cross-site mutations are denied.
